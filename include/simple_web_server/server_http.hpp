...
namespace SimpleWeb {
  ...
  protected:
    ...
    public:
      ...
      boost::asio::detail::mutex socket_close_mutex;

      std::unique_ptr<asio::deadline_timer> timer;

      void close() noexcept {
        error_code ec;
        std::unique_lock<boost::asio::detail::mutex> lock(socket_close_mutex); // The following operations seems to be needed to run sequentially
  ...
  public:
    ...
    void stop() noexcept {
      if(acceptor) {
        ...
        {
          std::unique_lock<boost::asio::detail::mutex> lock(*connections_mutex);
  ...
  protected:
    ...
    std::vector<boost::asio::detail::thread> threads;

    std::shared_ptr<std::unordered_set<Connection *>> connections;
    std::shared_ptr<boost::asio::detail::mutex> connections_mutex;

    std::shared_ptr<ScopeRunner> handler_runner;

    ServerBase(unsigned short port) noexcept : config(port), connections(new std::unordered_set<Connection *>()), connections_mutex(new boost::asio::detail::mutex()), handler_runner(new ScopeRunner()) {}
